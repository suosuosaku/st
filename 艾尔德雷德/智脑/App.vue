    ...
    <Transition name="zhino-panel">
      <div v-if="isPanelOpen" id="zhino-panel" class="zhino-panel" :class="{ mobile: isMobile }" :style="[panelStyle, { transform: `scale(${uiScale})`, transformOrigin: 'center center' }]">
        ...
        <div class="zhino-panel-content">
          ...
          <EldredTab v-else-if="currentTab === 'eldred'" />
...
import EldredTab from './components/EldredTab.vue';
...
const tabs = [
  ...
  { key: 'eldred' as const, label: '艾尔德雷德' },
...
const currentTab = ref<'overview' | 'world' | 'eldred' | 'relationship' | 'character' | 'dreamtalk' | 'timeline' | 'settings'>('overview');
