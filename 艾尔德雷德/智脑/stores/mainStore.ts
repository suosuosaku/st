...
import type {
  EldredWorldbookEnableBackup,
  EldredWorldbookInjectionReport,
  EldredWorldbookScan,
} from '../core/eldredWorldbookScheduler';
import { ELDRED_DEFAULT_ALWAYS_NAMES, ELDRED_DEFAULT_SCHEDULED_NAMES } from '../core/eldredWorldbookScheduler';
...
export interface ChatData {
  ...
  // 艾尔德雷德专属世界书调度
  eldredWorldbookScan: EldredWorldbookScan | null;
  eldredWorldbookLastInjection: EldredWorldbookInjectionReport | null;
  eldredWorldbookEnableBackups: EldredWorldbookEnableBackup[];
}
...
export interface ScriptSettings {
  ...
  settings: {
    ...
    // 艾尔德雷德专属世界书调度
    eldredWorldbookSchedulerEnabled: boolean;
    eldredWorldbookAutoInjectEnabled: boolean;
    eldredWorldbookAlwaysNames: string;
    eldredWorldbookScheduledNames: string;
    eldredWorldbookKeepEnabledNames: string;
    eldredWorldbookMaxEntries: number;
    eldredWorldbookMaxChars: number;
  };
}
...
const ChatDataSchema = z
  .object({
    ...
    eldredWorldbookScan: z.any().prefault(null),
    eldredWorldbookLastInjection: z.any().prefault(null),
    eldredWorldbookEnableBackups: z.array(z.any()).prefault([]),
  })
  .prefault({});
  ...
  .object({
    ...
    settings: z
      .object({
        ...
        // 艾尔德雷德专属世界书调度
        eldredWorldbookSchedulerEnabled: z.boolean().prefault(true),
        eldredWorldbookAutoInjectEnabled: z.boolean().prefault(true),
        eldredWorldbookAlwaysNames: z.string().prefault(ELDRED_DEFAULT_ALWAYS_NAMES),
        eldredWorldbookScheduledNames: z.string().prefault(ELDRED_DEFAULT_SCHEDULED_NAMES),
        eldredWorldbookKeepEnabledNames: z.string().prefault(''),
        eldredWorldbookMaxEntries: z.coerce.number().prefault(12),
        eldredWorldbookMaxChars: z.coerce.number().prefault(16000),
        // 小总结独立API（廉价模型，如DS）
