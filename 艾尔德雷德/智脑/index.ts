...
import {
  ...
  buildEldredWorldbookInjection,
  scanEldredWorldbooks,
} from './core/eldredWorldbookScheduler';
import {
  beautifyEldredMessage,
  beautifyVisibleEldredMessages,
  installEldredMessageBeautifierStyles,
} from './core/eldredMessageBeautifier';
...
$(() => {
  ...
  installEldredMessageBeautifierStyles();
  setTimeout(() => beautifyVisibleEldredMessages(), 0);
  ...
  async function refreshEldredWorldbookScan(store: ReturnType<typeof useMainStore>): Promise<void> {
    if (!store.settings.eldredWorldbookSchedulerEnabled) return;
    try {
      ...
      console.warn('[智脑-艾尔德雷德] 世界书扫描失败:', error);
    }
  }
  ...
  eventOn(tavern_events.MESSAGE_RECEIVED, (messageId, type) => {
    ...
    try {
      ...
      if (content) {
        ...
        beautifyEldredMessage(messageId, aiMsg.message);
  ...
  eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED, (messageId: number) => {
    setTimeout(() => beautifyEldredMessage(messageId), 0);
  });
  ...
  eventOn(tavern_events.CHAT_COMPLETION_SETTINGS_READY, async completion => {
    ...
    // ─── 艾尔德雷德世界书按名调度注入 ───
    if (
      store.settings.eldredWorldbookSchedulerEnabled
      && store.settings.eldredWorldbookAutoInjectEnabled
      && Array.isArray((completion as any).messages)
    ) {
      if (!store.chatData.eldredWorldbookScan?.entries?.length) {
        await refreshEldredWorldbookScan(store);
      }
      ...
      const injection = buildEldredWorldbookInjection(
        ...
        sceneText,
        buildEldredSchedulerConfig(store.settings),
      );
      if (injection) {
